...
export default function ObsSourcePage() {
  ...
  return <main className="force-dark grid min-h-screen place-items-center bg-gradient-to-br from-[#11284b] via-[#0a2942] to-[#020711]"><video ref={videoRef} autoPlay playsInline muted={muted} className="h-screen w-screen object-cover" />{status && <span className="absolute rounded bg-black/35 px-4 py-2 text-sm text-blue-100/70">{status}</span>}</main>;
}
