...
- In Paystack, set the webhook URL to `https://YOUR_FRONTEND_DOMAIN/api/payment/webhook`.
- Restrict public API keys to the intended domains and APIs where the providers support restrictions.

The payment API derives prices and credits on the server, verifies the Firebase ID token, validates Paystack signatures, and verifies the final amount and status before updating a wallet.
